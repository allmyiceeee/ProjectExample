...
item_user_matrix = ratings.pivot(
...
def reccomend_movie_item_based(
	movie_title, movies, item_user_matrix, item_similarity, n_recommendation=10
):
...
while True:
	...
	try:
		reccomend_movie_item_based = reccomend_movie_item_based(
			input_movie, movies, item_user_matrix, item_similarity
		)
		...
		if reccomend_movie_item_based:
			...
			print(
				f"Не найдено рекомендаций для {input_movie}. Возможно название фильма не найдено"
			)
